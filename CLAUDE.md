...
**Favourite tutors are the hard limit on what can be booked.** `/booking/favorites/calendar` only ever advertises the account's favourites, and the platform caps them (`limit: 5` in the `/favorite-tutors` response). A rule naming a tutor who is not favourited can therefore never book, however free that tutor actually is — and the failure reads "No suitable teachers available", identical to a genuinely full calendar. That ambiguity once hid four dead rules for days. `get_favorite_tutor_ids()` + `_unbookable_teachers()` name the cause in the log and the failure push; the check is diagnostic only, since a non-favourite cannot reach the candidate list anyway.

Favourites are **per account**, so they are fetched with the schedule's own client inside `_run_schedule`, alongside `get_bookings` and the calendar. Nothing account-scoped may be cached at module level. `data/teachers.json` is the exception and is safe to share: it maps name → id from `/tutors`, which is the platform-wide tutor list, not favourites.

**Teacher cache** (`app/teachers.py`): `data/teachers.json` (gitignored, project root) maps teacher name → `{id, status}`. Names are never deleted — absent teachers are marked `REMOVED`. Updated by `populate-teachers`. `_get_candidates` resolves only names whose status is `ACTIVE`: `populate_teachers` refreshes the id of a name the API still returns, so a `REMOVED` entry keeps its last known id — which the platform may since have reassigned. Resolving it would book a tutor the rule never named (tutor 4609 has already been renamed once, leaving `Maria Garcia` as a `REMOVED` alias for what is now `Mc Kaderin Quintero`). `run-due` checks the cache on startup: exits with a message if missing, and raises a `ValueError` if any name in the rules is unknown.
