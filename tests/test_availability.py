...
class TestGetAvailableTeachers:
    def test_returns_available_teachers(self, authed_client, calendar_response, tutors_response):
        client, router = authed_client
        router.get("/auth/tutors/list").mock(
            return_value=httpx.Response(200, json=tutors_response)
        )
        router.post("/auth/booking/calendar").mock(
            return_value=httpx.Response(200, json=calendar_response)
        )

        # 11:00 UTC on April 8 = 13:00 CEST
        result = get_available_teachers(client, "2026-04-08T11:00:00+00:00")
        ids = [t["id"] for t in result]
        assert "184" in ids
        assert "159" in ids

    def test_filters_booked_slots(self, authed_client):
        client, router = authed_client
        router.get("/auth/tutors/list").mock(
    ...
    def test_enriches_with_teacher_names(self, authed_client, calendar_response, tutors_response):
        client, router = authed_client
        router.get("/auth/tutors/list").mock(
            return_value=httpx.Response(200, json=tutors_response)
        )
    ...
    def test_unknown_teacher_defaults_to_id(self, authed_client):
        client, router = authed_client
        router.get("/auth/tutors/list").mock(
    ...
    def test_api_failure_returns_empty(self, authed_client):
        client, router = authed_client
        router.get("/auth/tutors/list").mock(return_value=httpx.Response(200, json={"data": []}))
        router.post("/auth/booking/calendar").mock(return_value=httpx.Response(500, text="Server Error"))

        result = get_available_teachers(client, "2026-04-08T11:00:00+00:00")
        assert result == []

    def test_handles_list_response_format(self, authed_client):
        """
    ...
    def test_local_time_in_result_cest(self, authed_client):
        client, router = authed_client
        router.get("/auth/tutors/list").mock(
            return_value=httpx.Response(200, json={"data": [{"id": 184, "name": "Maria", "is_favorite": False}]})
        )
    ...
    def test_local_time_in_result_cet(self, authed_client):
        client, router = authed_client
        router.get("/auth/tutors/list").mock(
            return_value=httpx.Response(200, json={"data": [{"id": 184, "name": "Maria", "is_favorite": False}]})
        )
        ...
        result = get_available_teachers(client, "2026-01-08T12:00:00+00:00")
        assert result[0]["start_time_local"] == "13:00"

    def test_no_match_at_different_time(self, authed_client, calendar_response, tutors_response):
        client, router = authed_client
        router.get("/auth/tutors/list").mock(return_value=httpx.Response(200, json=tutors_response))
        router.post("/auth/booking/calendar").mock(return_value=httpx.Response(200, json=calendar_response))

        # Request a different time — no slots at 14:00 UTC
        result = get_available_teachers(client, "2026-04-08T14:00:00+00:00")
        assert result == []
...
class TestGetTeacherSlots:
    def test_fetches_slots_for_teacher_string_id(self, authed_client, calendar_response):
        client, router = authed_client
        router.post("/auth/booking/calendar").mock(
            return_value=httpx.Response(200, json=calendar_response)
        )

        slots = get_teacher_slots(client, "184")
        assert len(slots) == 3
        assert slots[0]["start_time"] == "2026-04-08T11:00:00+00:00"

    def test_fetches_slots_for_teacher_int_id(self, authed_client, calendar_response):
        client, router = authed_client
        router.post("/auth/booking/calendar").mock(
            return_value=httpx.Response(200, json=calendar_response)
        )
    ...
    def test_returns_empty_on_http_error(self, authed_client):
        client, router = authed_client
        router.post("/auth/booking/calendar").mock(return_value=httpx.Response(500))

        assert get_teacher_slots(client, "184") == []

    def test_returns_empty_for_unknown_teacher(self, authed_client, calendar_response):
        client, router = authed_client
        router.post("/auth/booking/calendar").mock(
            return_value=httpx.Response(200, json=calendar_response)
        )

        assert get_teacher_slots(client, "9999") == []
