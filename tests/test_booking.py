...
class TestBookLessonPayload(BaseTest):
    ...
    def _get_payload(self, lesson_datetime):
        ...
        self.router.post("/auth/booking/new-add").mock(side_effect=capture)
        book_lesson(self.mock_client, "184", lesson_datetime)
        return captured["body"]

    def test_payload_summer_cest(self):
        """13:00 CEST (UTC+2) → date stays Madrid, times in UTC (11:00/11:30)."""
        payload = self._get_payload("2026-04-08T13:00:00+02:00")
    ...
    def test_payload_winter_cet(self):
        ...
        payload = self._get_payload("2026-01-08T13:00:00+01:00")
    ...
    def test_payload_utc_z_input(self):
        ...
        payload = self._get_payload("2026-04-08T11:00:00Z")

        assert payload["date"] == "2026-04-08"
        assert payload["start_time"] == "11:00"
        assert payload["end_time"] == "11:30"

    def test_payload_date_is_madrid_not_utc(self):
        """
    ...
    def test_number_of_people_is_1(self):
        payload = self._get_payload("2026-04-08T13:00:00+02:00")
    ...
    def test_teacher_id_coerced_to_string(self):
        ...
        self.router.post("/auth/booking/new-add").mock(side_effect=capture)
        book_lesson(self.mock_client, 184, "2026-04-08T13:00:00+02:00")
        assert captured["body"]["staff_id"] == "184"

    def test_timezone_field_is_madrid(self):
        payload = self._get_payload("2026-04-08T13:00:00+02:00")
        assert payload["timezone"] == "Europe/Madrid"

    def test_type_of_class_field(self):
        payload = self._get_payload("2026-04-08T13:00:00+02:00")
    ...
    def test_end_time_is_30_min_after_start(self):
        ...
        payload = self._get_payload("2026-04-08T16:30:00+02:00")
        # 16:30 CEST = 14:30 UTC; end = 15:00 UTC
        assert payload["start_time"] == "14:30"
        assert payload["end_time"] == "15:00"

    def test_returns_api_response_on_success(self):
        self.router.post("/auth/booking/new-add").mock(
            return_value=httpx.Response(200, json={"status": "success", "id": "42"})
        )

        result = book_lesson(self.mock_client, "184", "2026-04-08T13:00:00+02:00")
        assert result == {"status": "success", "id": "42"}

    def test_returns_error_dict_on_http_failure(self):
        self.router.post("/auth/booking/new-add").mock(
...
class TestGetBookings(BaseTest):
    ...
    def test_returns_data_list_on_success(self, bookings_response):
...
class TestCancelBooking(BaseTest):
    ...
    def test_cancel_http_error(self):
        self.router.post("/auth/booking/cancel/9999").mock(
